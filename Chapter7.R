# Mussina Heat map of tOPS+ through each balls/strikes count
mussina = expand.grid(balls=0 : 3, strikes=0: 2)
mussina$value = c(100,118,157,207,72,82,114,171,30,38,64,121)
mussina
...
# Derek Holland's Career Heat Map of tOPS+ through each balls/strikes count
holland = expand.grid(balls = 0:3, strikes = 0:2)
holland$value = c(100,125,178,311,74,91,130,238,35,43,82,153)
countmap(holland)

names(pbp2011) = fields$Header
pbp2011$pseq = gsub("[.>123N+*]","",pbp2011$PITCH_SEQ_TX)
# identifying a 1-0 count
pbp2011$c10 = grepl("^[BIPV]", pbp2011$pseq)
# identifying an 0-1 count
...
# Mean Run Value for at-bats starting with a ball vs with a strike
ab10 = subset(pbp11rc, c10 == 1)
ab01 = subset(pbp11rc, c01 == 1)
c(mean(ab10$RUNS.VALUE), mean(ab01$RUNS.VALUE))

# Runs Value for Plate Appearances through each balls/strikes count
...
# How Miguel Cabrera alters his swinging tendancies according to the ball/strike count
...
outKzone = 0.95
library(lattice)
xyplot(pz~px, data=sampCabrera, groups=swung, aspect = "iso",
...
# How Miguel Cabrera alters his swinging tendancies according to the pitch location
...
migg00loess = loess(swung ~ px + pz, data = miggy00, control = loess.control(surface = "direct"))
pred.area$fit00 = c(predict(migg00loess,pred.area))
...
verl_type_cnt_L = table(verl_LHB$bscount, verl_LHB$pitch_type)
