...
with(Mantle, plot(Age,OPS,cex=1,pch=19))

# Curve: A + B(Age-30) + C(Age-30)^2
...
# Curve: 0.955-0.0202(Age-30)-0.00352(Age-30)^2
...
summary(F2$fit)
# 61% of the variability in Mantle's OPS values can be explained by the curve
# Approx 2/3 of the residuals (vertical deviations) falls within 0.075 from the curve
 ...
 = = function(playerid){
  ...
  F2 = merge(F2, 
              data.frame(playerID = player.list,
                         Name = sapply(as.character(player.list),=)))
...
d = plot.traj2("Mickey","Mantle",6,3)
d = plot.traj2("Derek", "Jeter", 6,3)
...
# Player's Peak Age and how this has changed over time 
midcareers = ddply(Batting.2000,.(playerID),summarize,Midyear=(min(yearID)+max(yearID)/2))
...
# Relationship between a player's peak age and career AB's
with(Beta.coef, 
...
# Comparing players categorized by position
